...
Current-state rows below were checked on **2026-09-07**. They distinguish
repository exists, code merged on `main`, PR pending, deployed service, and
tested behavior. An open PR or a passing check is none of the last three.

## Deployment units

| # | Unit | Ships as | Runs on | Status (2026-09-07) |
|---|---|---|---|---|
...
| ② | The scaffold (`vllm-ascend-workspace`) | one clone/fork per developer | developer machine | shipped as [`maoxx241/vllm-ascend-workspace`](https://github.com/maoxx241/vllm-ascend-workspace) (`main` `161fed1b`); **org transfer pending** (`vllm-ascend-workspace/vllm-ascend-workspace` is not present) |
| ③ | `vaws-coordinator` | loopback HTTP MCP service, one shared manager process; stdio `vaws_*` provider is a separate server | a machine the developers can reach over loopback or an SSH tunnel | [`vaws-coordinator`](https://github.com/vllm-ascend-workspace/vaws-coordinator) exists (public); `main` `f7c0682b` has the HTTP manager, pool and jobs; stdio `vaws_*` is [#2](https://github.com/vllm-ascend-workspace/vaws-coordinator/pull/2) (**open**, not on `main`); hardware-level guarantees **not established** |
| ④ | `remote-dev` | stdio MCP server + CLI fallbacks | developer machine, talks SSH outward | [`remote-dev`](https://github.com/vllm-ascend-workspace/remote-dev) exists (private / access-limited); `main` `2ff11629` has the tool surface; scaffold `main` still ships `.remote-dev/` until [#90](https://github.com/maoxx241/vllm-ascend-workspace/pull/90) merges; live SSH re-validation of the standalone tree **not claimed** |
| ⑤ | `vaws-top` | loopback web UI + API, plus a read-only stdio MCP | developer machine or a bastion; systemd user service or a Windows scheduled task | [`vaws-top`](https://github.com/vllm-ascend-workspace/vaws-top) exists (private / access-limited); `main` `e1347848` has collection, dashboard and read-only MCP; observation only; **not** a device-allocation authority; not claimed deployed |
| ⑥ | `vaws-knowledge` | Git repo + knowledge MCP engine; review bot not on `main` | GitHub, plus a local engine per consumer | [`vaws-knowledge`](https://github.com/vllm-ascend-workspace/vaws-knowledge) exists (public); `main` `5051b115` has schema plus `tools/` `server/` `sync/` `conformance/` ([#1](https://github.com/vllm-ascend-workspace/vaws-knowledge/pull/1)–[#4](https://github.com/vllm-ascend-workspace/vaws-knowledge/pull/4) merged); `bot/` is still `.gitkeep` ([#5](https://github.com/vllm-ascend-workspace/vaws-knowledge/pull/5) **open**); corpus empty |
...
[#2](https://github.com/vllm-ascend-workspace/vaws-coordinator/pull/2) and was
**open** on 2026-09-07 — it is not on `main`. Accepted stdio is
newline-delimited JSON (legacy Content-Length may also work). Capability
authority is `capabilities.experimental`, not undocumented `serverInfo` fields.
...
flowchart TB
  ...
  subgraph U3["③ vaws-coordinator · loopback HTTP MCP"]
    ...
    STDIO["stdio vaws tools provider<br/><small>own server, not the HTTP manager<br/>PR 2 open — not on main</small>"]
  end
  ...
  PRIV -- "C source-side redaction + one-way PR<br/>(tools on main; review bot #5 open)" --> KBOT
  KBOT --> KGATE
  KGATE --> KSHARE
  KSHARE --> KENG
...
sync and conformance kit are on `main`; the review bot is not.
...
4. **Redact at the source, then one PR.** Redaction runs in the contributing
   ...
   Scaffold export/migration onto that contract is [#84](https://github.com/maoxx241/vllm-ascend-workspace/pull/84) (**open**).
5. **Bot, then a human.** The bot can gate schema, redaction re-scan, exact and
   ...
   Bot validation and conflict marking do not prove technical truth. The review
   bot itself is [#5](https://github.com/vllm-ascend-workspace/vaws-knowledge/pull/5) (**open**);
   `bot/` on `main` is still a `.gitkeep`. There is no `CODEOWNERS` file on
   `vaws-knowledge` `main`; protection settings were not read, and are not
   claimed.
6. **Back into queries.** Forks periodically pull `verified` downward into a
   ...
   than silently confident. Shared / project / candidate are trust layers,
   distinct from the verified / unverified review zones.
...
The legend distinguishes merged from pending from absent; this table is the
evidence behind it, checked on **2026-09-07**. Full SHAs are in the Basis
column. "On `main`" means the named tree at that SHA. It does not mean
deployed, and it does not mean a hardware or client pass.
